...
describe('import conversion for non-native containers (real ffmpeg)', () => {
  ...
  // ts/m2ts are deliberately absent: the bundled ffmpeg-static build
  // SIGSEGVs on any muxed output from an mpegts input (remux AND
  // transcode; decode-to-null is fine). This matrix is what caught it.
  // See KNOWN_UNSUPPORTED_VIDEO_EXTENSIONS in shared/mediaFormats.ts.
  const CONTAINERS: Array<{ ext: string; extraArgs: string[] }> = [
    { ext: 'flv', extraArgs: [] },
    { ext: 'wmv', extraArgs: [] },
    { ext: 'mpg', extraArgs: [] },
  ...
  for (const { ext, extraArgs } of CONTAINERS) {
    it(`${ext}: converts to a playable faststart h264/aac mp4`, async () => {
